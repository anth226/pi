...
class mailEclipse
{
    ...
    public static function deleteTemplate($templateSlug)
    {
        ...
        if (! is_null($template)) {
	        ...
	        self::getTemplates()
	            ->where('template_slug', $templateSlug)->delete();
	...
	public static function deleteTemplate_v1($templateSlug)
	{
		$template = self::getTemplates()
		                ->where('template_slug', $templateSlug)->first();
    ...
    public static function updateTemplate($request)
    {
        ...
        if (! is_null($template)) {
            ...
            //
            //
			$templ =  self::getTemplates()->where('template_slug', $templatename);
            if ($templ) {
                return response()->json([
	...
	public static function updateTemplate_v1($request)
	{
		$template = self::getTemplates()
		                ->where('template_slug', $request->templateslug)->first();
    ...
    public static function getTemplates_v1()
    {
	...
	public static function getTemplatesPaginated(){
    	return self::getTemplates()->sortable( [ 'id' => 'desc' ] )->paginate( 10 );
	}
    ...
    public static function createTemplate($request)
    {
        ...
        if (! view()->exists($view) && ! self::getTemplates()->where('template_slug', $templatename)->count()) {
            self::saveTemplates(
                [
	...
	public static function createTemplate_v1($request)
	{
		if (! preg_match("/^[a-zA-Z0-9-_\s]+$/", $request->template_name)) {
			return response()->json([
    ...
    public static function markdownedTemplateToView($save = true, $content = '', $viewPath = '', $template = false)
    {
	    ...
	    $user_logged = Auth::user();
        $temp_path = base_path().'/resources/views/vendor/maileclipse/templates/';
		$slug = str_replace([$temp_path,'.blade.php'], ['',''], $viewPath);
	...
	public function import(){
    	$res = json_decode(file_get_contents(self::getTemplatesFile()), 1);
    	if($res && count($res)){
    		foreach($res as $r){
			    EmailTemplates::create($r);
		    }
	    }
		dd($res);
	}
